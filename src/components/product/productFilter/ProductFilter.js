...
const ProductFilter = () => {
  ...
  // const [price, setPrice] = useState(maxPrice > 3000 ? maxPrice : 3000);
    ...
    <div className={styles.filter}>
      ...
      <div className={styles.brand}>
        <select value={brand} onChange={(e)=>setBrand(e.target.value)}>
          {allBrands.map((brandValue,index)=>{
            return(
              <option key={index} value={brandValue}>{brandValue}</option>
            )
          })}
        </select>
        <h4>Price</h4>
        ...
        <br/>
        <button className='--btn --btn-danger' onClick={clearFilters}>Cleare Filter</button>
      </div>
    </div>
  )
}
